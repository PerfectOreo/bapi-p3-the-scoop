...
function createComment(url, request){ //Going to copy and paste and edit createArticle
  const requestComment = request.body && request.body.comment;
  const response = {};
  const commentsArticleId = Number(url.split('/').filter(segment => segment)[1]);

    if(requestComment && database.users[requestComment.username] && database.articles[requestComment.articleId] && requestComment.body.length !== 0){ //Checks if username exists, article exists, and comment actually has comemnt instead of a blank
      const comment = {
    ...
    database.articles[comment.articleId].commentIds.push(comment.id); //Adds comment id under the article
    database.users[comment.username].commentIds.push(comment.id); //Adds comment id under user
    database.comments[comment.id] = comment; //Adds comment to database

    response.body = {comment: comment}; //Replaced article with comment
    response.status = 201;
  }else {
    response.status = 400;
  }
...
function updateComment(url, request){ //Using updateArticle as reference or copy/paste/edit
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id]; //Retreiving comment to be edited
  const requestComment = request.body && request.body.comment; //What comment should be replaced with or turn into
  const response = {};

  if (!id || !requestComment || requestComment.body.length === 0) { //Makes sure comment id and what the comment will be changed into exist, also checks that comment isn't blank
    response.status = 400;
  } else if (!savedComment) { //Make sure the comment being edited actually exists
    response.status = 404;
  } else {
    savedComment.body = requestComment.body || savedArticle.body; // Is this not a true/false statement?
...
function deleteComment(url, request){ //deleteArticle was used as CPE material
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id]; //Finds comment that will be deleted
  const response = {};

  if (savedComment) {
    const articleWith1LessCommentIdNow = database.articles[savedComment.articleId].commentIds; //Succint but useful names are hard
    articleWith1LessCommentIdNow.splice(articleWith1LessCommentIdNow.indexOf(id), 1); //Removes comment id from article
    database.comments[id] = null; //makes the comment empty
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1); //removes commentId from user
    response.status = 204;
  } else {
    response.status = 404;
  }
...
function upvoteComment(url, request){ //Guess what was used as CPE material
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  ...
  if (savedComment && database.users[username]) {
    savedComemnt = upvote(savedComment, username); //Thank god this function exists and double checks stuff
